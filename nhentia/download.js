  ...
  try {
    ...
    for (const hrefElement of findHref) {
      ...
      try {
        ...
        // download !!!!!
        await asyncdownload(img, Headers, `./${title}`);
      } catch (err) {
        console.log(err);
      }
    }
...
async function asyncdownload(urls, path) {
  try {
    const downloadPromises = urls.map((url) => downloadimg(url, Headers, path));
    await Promise.all(downloadPromises);
    console.log('所有漫畫下載完成！');
  } catch (error) {
    console.error('發生錯誤(多線呈)', error);
  }
}
